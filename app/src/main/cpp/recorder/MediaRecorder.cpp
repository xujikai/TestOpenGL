...
int MediaRecorder::StartRecord() {
    ...
    do {
        ...
        avformat_alloc_output_context2(&m_FormatCtx, nullptr, nullptr, m_OutUrl);
        if (!m_FormatCtx) {
            LOGCATE("MediaRecorder::StartRecord Could not deduce output format from file extension: using MPEG.\n");
            avformat_alloc_output_context2(&m_FormatCtx, nullptr, "mpeg", m_OutUrl);
        }
        if (!m_FormatCtx) {
            result = -1;
            break;
        ...
        if (!(m_OutputFormat->flags & AVFMT_NOFILE)) {
            ...
            if (ret < 0) {
                LOGCATE("MediaRecorder::StartRecord Could not open '%s': %s", m_OutUrl, av_err2str(ret));
                result = -1;
                break;
            }
        }
        ...
        if (result < 0) {
            LOGCATE("MediaRecorder::StartRecord Error occurred when opening output file: %s", av_err2str(result));
            result = -1;
            break;
        }
    ...
    *codec = avcodec_find_encoder(codec_id);
    ...
    switch ((*codec)->type) {
        case AVMEDIA_TYPE_AUDIO:
            ...
            codecContext->sample_rate = m_RecorderParam.audioSampleRate;
            codecContext->channel_layout = m_RecorderParam.channelLayout;
            codecContext->channels        = av_get_channel_layout_nb_channels(codecContext->channel_layout);
            ost->m_pStream->time_base = (AVRational){1, codecContext->sample_rate };
            break;

        case AVMEDIA_TYPE_VIDEO:
            LOGCATE("MediaRecorder::AddStream AVMEDIA_TYPE_VIDEO AVCodecID=%d", codec_id);
            codecContext->codec_id = codec_id;
            codecContext->bit_rate = m_RecorderParam.videoBitRate;
            /* Resolution must be a multiple of two. */
            codecContext->width    = m_RecorderParam.frameWidth;
            codecContext->height   = m_RecorderParam.frameHeight;
            /* timebase: This is the fundamental unit of time (in seconds) in terms
             * of which frame timestamps are represented. For fixed-fps content,
             * timebase should be 1/framerate and timestamp increments should be
             * identical to 1. */
            ost->m_pStream->time_base = (AVRational){ 1, m_RecorderParam.fps };
            codecContext->time_base       = ost->m_pStream->time_base;
    ...
    if(frame != nullptr) {
        ...
        * internally; make sure we do not overwrite it here */
        ...
        if (srcPixFmt != AV_PIX_FMT_YUV420P) {
            ...
            if (!ost->m_pSwsCtx) {
                LOGCATE("MediaRecorder::EncodeVideoFrame src width=%d, height=%d, format=%d", frame->width, frame->height, srcPixFmt);
                LOGCATE("MediaRecorder::EncodeVideoFrame dst width=%d, height=%d, format=%d", c->width, c->height, c->pix_fmt);
                ost->m_pSwsCtx = sws_getContext(frame->width, frame->height, srcPixFmt,
            ...
            sws_scale(ost->m_pSwsCtx, (const uint8_t * const *) frame->data,
                      frame->linesize, 0, frame->height, ost->m_pFrame->data,
