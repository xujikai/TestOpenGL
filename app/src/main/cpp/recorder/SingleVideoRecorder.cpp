...
int SingleVideoRecorder::StartRecord() {
    ...
    if (result >= 0) {
        m_encodeThread = new std::thread(StartH264EncoderThread, this);
    }
    return result;
}
...
int SingleVideoRecorder::OnFrame2Encode(NativeImage *inputFrame) {
    if (m_exit) return 0;
    LOGCATE("SingleVideoRecorder::OnFrame2Encode format=%d, width=%d, height=%d", inputFrame->format, inputFrame->width, inputFrame->height);
    LOGCATE("SingleVideoRecorder::OnFrame2Encode line0=%d, line1=%d, line2=%d", inputFrame->pLineSize[0], inputFrame->pLineSize[1], inputFrame->pLineSize[2]);
        ...
        if (srcPixFmt != AV_PIX_FMT_YUV420P) {
            ...
            if(recorder->m_SwsContext != nullptr) {
                int slice = sws_scale(recorder->m_SwsContext, pImage->ppPlane, pImage->pLineSize, 0,
                                      pImage->height, pFrame->data, pFrame->linesize);
