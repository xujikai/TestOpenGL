...
MyEGLRenderContext::MyEGLRenderContext() {
...
void MyEGLRenderContext::Init() {
    if (CreateGLESEnv() == 0) {
        m_IsGlContextReady = true;
    }
    if (!m_IsGlContextReady) return;

    m_ShaderStrArr[0] = fShaderStr0;
    m_ShaderStrArr[1] = fShaderStr1;
    m_ShaderStrArr[2] = fShaderStr2;
    m_ShaderStrArr[3] = fShaderStr3;
    m_ShaderStrArr[4] = fShaderStr4;
    m_ShaderStrArr[5] = fShaderStr5;
    m_ShaderStrArr[6] = fShaderStr6;
    m_ShaderStrArr[7] = fShaderStr7;

    m_ProgramObj = GLUtils::CreateProgram(vShaderStr, m_ShaderStrArr[m_ShaderIndex], m_VertexShader, m_FragmentShader);
    if (!m_ProgramObj) {
    ...
    glVertexAttribPointer(VERTEX_POS_LOC, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), nullptr);
    glBindBuffer(GL_ARRAY_BUFFER, GL_NONE);
    glBindBuffer(GL_ARRAY_BUFFER, m_VboIds[1]);
    glEnableVertexAttribArray(TEXTURE_POS_LOC);
    glVertexAttribPointer(TEXTURE_POS_LOC, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(GLfloat), nullptr);
    glBindBuffer(GL_ARRAY_BUFFER, GL_NONE);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_VboIds[2]);
...
void MyEGLRenderContext::UnInit() {
    if (m_ProgramObj) {
...
int MyEGLRenderContext::CreateGLESEnv() {
...
void MyEGLRenderContext::DestroyGLESEnv() {
    // 8. 释放 EGL 环境
    if (m_EglDisplay != EGL_NO_DISPLAY) {
...
void MyEGLRenderContext::SetImageData(int width, int height, uint8_t *pData) {
...
MyEGLRenderContext *MyEGLRenderContext::GetInstance() {
    if (m_Instance == nullptr) {
        m_Instance = new MyEGLRenderContext();
    }
    return m_Instance;
}

void MyEGLRenderContext::DestroyInstance() {
    if (m_Instance) {
        delete m_Instance;
        m_Instance = nullptr;
    }
